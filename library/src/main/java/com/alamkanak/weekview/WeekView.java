...
public class WeekView extends View {
    ...
    private GestureDetector.SimpleOnGestureListener defaultmGestureListener = new GestureDetector.SimpleOnGestureListener() {
    ...
    private GestureDetector.SimpleOnGestureListener mGestureListener = defaultmGestureListener;
     ...
     * @param numberOfVisibleDays The number of visible days in a week.
     */
    public void setNumberOfVisibleDaysInPlace(int numberOfVisibleDays) {
        Calendar c = getDayFromPosition();
        goToDate(c);
        this.mNumberOfVisibleDays = numberOfVisibleDays;
    }
    ...
    /**
     * Show a specific day in the middle of the calendar.
     * @param date The date to show.
     */

    public void goToMidlleCalendarDate(Calendar date) {
        mScroller.forceFinished(true);
        mCurrentScrollDirection = mCurrentFlingDirection = Direction.NONE;
        ...
        mCurrentOrigin.x = (-dateDifference + dayoffset) * (mWidthPerDay + mColumnGap);
        invalidate();
    }

    /**
     * Show next week on the week view.
     */
    public void goToNextWeek(){
        //get the abscisse, hence current today minus position
        Calendar c = getDayFromPosition();
    ...
    public void goToNextDays(){
        //get the abscisse, hence current today minus position
        Calendar c = getDayFromPosition();
        int posDay = c.get(Calendar.DAY_OF_WEEK);
        long timeInMillis=0;
        long day = 1000L * 60L * 60L * 24L;
        ...
        switch (getNumberOfVisibleDays()){
            case 7:
                ...
                if (posDay == 2){
                    ...
                    deltaDays = (7-(posDay-2>0?posDay-2:posDay-2==-1? 6:0))*day;
                }
                timeInMillis = c.getTimeInMillis() + deltaDays;
                break;
            case 3:
                timeInMillis = c.getTimeInMillis() + 3*day;
                break;
            case 1:
                timeInMillis = c.getTimeInMillis() + day;
                break;
        }
    ...
    /**
     * Show previous week on the week view.
     */
    ...
    public void goToPreviousDays(){
        ...
        switch (getNumberOfVisibleDays()){
            case 7:
                ...
                if (posDay == 2){
                    ...
                    deltaDays = (posDay-2>0?posDay-2:posDay-2==-1? 6:0)*day;
                }
                timeInMillis = c.getTimeInMillis() - deltaDays;
                break;
            case 3:
        ...
        c.setTimeInMillis(timeInMillis);
        // bypass the issues of leap year and parityof months
     ...
     * Show next week on the week view.
     */
    public Calendar getDayFromPosition (){
        //get the abscisse, hence current today minus position
        Calendar today = Calendar.getInstance();
        ...
        long day = 1000L * 60L * 60L * 24L;
        long todayInMillis = today.getTimeInMillis() + today.getTimeZone().getOffset(today.getTimeInMillis());
        double joursecart = mCurrentOrigin.x/(mWidthPerDay + mColumnGap);
        long dateInMillis = (todayInMillis - Math.round(joursecart)*day);
     ...
     * Show next week on the week view.
     */
    public Calendar getMidlleCalendarDayFromPosition (){
        //get the abscisse, hence current today minus position
        Calendar today = Calendar.getInstance();
        ...
        long dateInMillis = (todayInMillis - Math.round(joursecart - dayoffset)*day);
        Calendar date = Calendar.getInstance();
        date.setTimeInMillis(dateInMillis);
        return date;
    }
...
//                    break;
        ...
        mGestureDetector = new GestureDetectorCompat(mContext, simpleOnGestureListener);
    }
    public void unlockScroll(){
        mGestureDetector = new GestureDetectorCompat(mContext, defaultmGestureListener);
    }
    /**
